...
%
function deriv = HW4_grad_vec(x,delx,n_of_var)
...
xvec2(i) = x(i) + delx; % change i-th DV only.
   if (ideriv == 0),
    xvec1(i) = x(i) - delx;
    deriv(i) = (HW4_func_multivar(xvec2) - HW4_func_multivar(xvec1))/(2*delx);
   else
    deriv(i) = (HW4_func_multivar(xvec2) - HW4_func_multivar(x) )/(delx);
   end
