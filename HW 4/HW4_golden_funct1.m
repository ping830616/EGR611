...
% 
function [alpha1,falpha1] = HW4_golden_funct1(x,search)
...
fa=HW4_func_multivar(x+a*search); n=1;
b=a+addx; fb=HW4_func_multivar(x+b*search);
   while (fb < fa), % the idea is to iterate until fb > fa in this case the minimum is in between a and b
     fa=fb;
     addx=addx*adddx_mult; b=b+addx;
     fb=HW4_func_multivar(x+b*search);
     n=n+1;
   end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tau = (3-sqrt(5))/2;
...
falpha1 = HW4_func_multivar(x+alpha1*search);
falpha2 = HW4_func_multivar(x+alpha2*search);
